...
class TwoWindowFD:
    def __init__(self, sw_size=10, lw_size=100, safety_margin=1.5):
        ...
        self.latency_map = {(sw, cp): 5.0 for sw in SWITCHES for cp in CONTROLLERS}
...
class SAEF_Orchestrator:
    def __init__(self):
        ...
        self.beta = 0.5
        self.lbm_history = []
        self.fd_monitors = {cp: TwoWindowFD(safety_margin=3.0) for cp in CONTROLLERS}
        self.last_heartbeat_time = {cp: time.time() for cp in CONTROLLERS}
        self.lock = threading.Lock() # Plan bankasına erişim güvenliği için
        self.latency_lock = threading.Lock()   # Gecikme tablosu için kilit
        self.latency_map = {(sw, cp): 5.0 for sw in SWITCHES for cp in CONTROLLERS} # Gecikme tablosu
        self.plan_bank["NORMAL"] = {
            "00:00:00:00:00:00:00:01": "c1",
            "00:00:00:00:00:00:00:02": "c2",
            "00:00:00:00:00:00:00:03": "c3"
        }

    def get_sw_name(self, dpid): return SW_NAMES.get(dpid, dpid)
    def get_cp_name(self, cp): return CP_NAMES.get(cp, cp)

    def latency_monitor_loop(self):
        """Arka planda gecikmeleri ölçer ve latency_map'i günceller."""
        while True:
            for sw in SWITCHES:
                for cp in self.active_controllers:
                    val = self.measure_real_latency(sw, cp)
                    with self.latency_lock:
                        self.latency_map[(sw, cp)] = val
            time.sleep(1) # Ölçümler arası nefes payı

    def measure_real_latency(self, sw_id, target_cp):
        """Gerçek HTTP isteği atan kısım (Sadece monitor thread çağırır)"""
        try:
            ...
            response = requests.get(url, timeout=1.0) # Timeout'u biraz kısalttık
            if response.status_code == 200:
                return max((time.time() - start_time) * 1000.0, 0.1)
            return 200.0
        except:
            return 300.0
    ...
    def get_latency(self, sw_id, target_cp):
        """Hafızadaki (cache) gecikme değerini döner. Çok hızlıdır."""
        with self.latency_lock:
            return self.latency_map.get((sw_id, target_cp), 100.0)
    ...
    def cost_function(self, plan, current_active_list):
        ...
        # --- EKLEME: Sticky Assignments (Gereksiz taşımayı engelle) ---
        migration_penalty = 0
        # Mevcut plan bankasındaki "Normal" durumu referans al
        current_actual = self.plan_bank.get("NORMAL", {})
        if current_actual:
            for sw, cp in plan.items():
                if sw in current_actual and cp != current_actual[sw]:
                    migration_penalty += 0.5 # Her gereksiz taşıma için %50 maliyet cezası
        
        return (self.alpha * norm_delay) + (self.beta * norm_lsd) + migration_penalty
    ...
    def get_load(self, url):
        try:
            # 1. Bellek (Memory) Verisini Çek ve Oranla
            mem_r = requests.get(f"http://{url}/wm/core/memory/json", timeout=1.5).json()
            total_mem = mem_r.get('total', 1)
            free_mem = mem_r.get('free', 0)
            # Bellek kullanım oranı (0.0 - 1.0 arası)
            mem_usage = (total_mem - free_mem) / total_mem

            # 2. Trafik Yükünü Çek (Senin mevcut mantığın)
            stat_r = requests.get(f"http://{url}/wm/statistics/bandwidth/all/all/json", timeout=1.5).json()
            traffic_load = sum(int(d['bits-per-second-rx']) for d in stat_r) / 50000.0

            # 3. Hibrit Yük Skoru (Literatür Katkısı: Memory + Traffic)
            # Bellek doluluğuna %60, Trafiğe %40 ağırlık veriyoruz.
            # Çünkü Java tabanlı Floodlight'ta bellek şişmesi trafikten daha tehlikelidir.
            hybrid_load = (mem_usage * 0.6) + (traffic_load * 0.4)
            
            # Loglamak istersen (isteğe bağlı):
            # logger.debug(f"[{url}] Mem:%{mem_usage*100:.1f} Traffic:{traffic_load:.2f} -> Hybrid:{hybrid_load:.2f}")
            
            return max(hybrid_load, 0.05)
        except Exception as e:
            # logger.error(f"Yük çekme hatası ({url}): {e}")
            return None

    def get_current_master(self, sw_id):
        # Bu bilgi plan_bank["NORMAL"] içinde tutuluyor olmalı
        normal_plan = self.plan_bank.get("NORMAL", {})
        return normal_plan.get(sw_id)

    def trigger_recovery(self, failed_cp, start_time):
        logger.warning(f"=== [SAEF NOKTA ATIŞI - DUP ENGELLEME BAŞLADI] ===")
        
        # 1. Sabit Harita (Bunu fonksiyon içinde tutmak en güvenlisi)
        actual_mapping = {
            "00:00:00:00:00:00:00:01": "192.168.56.107:6653", # s1: c1
            "00:00:00:00:00:00:00:02": "192.168.56.107:7753", # s2: c2
            "00:00:00:00:00:00:00:03": "192.168.56.107:8853"  # s3: c3
        }

        with self.lock:
            recovery_plan = self.plan_bank.get(f"FAIL_{failed_cp}")
            # c1'i kesinlikle dışla
            alive_cps = [cp for cp in CONTROLLERS if cp in self.active_controllers and cp != failed_cp]
        
        if not recovery_plan: return

        for sw in SWITCHES:
            # Sadece master'ı c1 olan switch (s1) için işlem yap
            if actual_mapping.get(sw) == failed_cp:
                try:
                    target_cp = recovery_plan.get(sw)
                    if not target_cp: continue

                    # A. YENİ MASTER'I ATA
                    t_idx = CONTROLLERS.index(target_cp)
                    m_url = f"http://192.168.56.107:{REST_PORTS[t_idx]}/wm/core/switch/{sw}/role/json"
                    requests.post(m_url, json={"role": "MASTER"}, timeout=0.5)
                    logger.info(f"|-- [SUCCESS] {self.get_sw_name(sw)} Yeni Master: {self.get_cp_name(target_cp)}")

                    # B. KRİTİK: DİĞER TÜM CANLILARI 'EQUAL' YAPARAK SUSTUR
                    # Bu adım (DUP!) paketlerini anında keser.
                    for other_cp in alive_cps:
                        if other_cp != target_cp:
                            o_idx = CONTROLLERS.index(other_cp)
                            e_url = f"http://192.168.56.107:{REST_PORTS[o_idx]}/wm/core/switch/{sw}/role/json"
                            # Floodlight/OpenFlow'da 'EQUAL' veya 'OTHER' rölü paketi işleme izni vermez.
                            requests.post(e_url, json={"role": "EQUAL"}, timeout=0.4)
                            logger.info(f"|-- [SILENCED] {self.get_sw_name(sw)} Equal yapıldı: {self.get_cp_name(other_cp)}")

                except Exception as e:
                    logger.error(f"|-- {sw} hatası: {e}")
            else:
                # s2 ve s3 zaten canlı denetleyicilerde olduğu için ellenmez
                logger.info(f"|-- [SAFE] {self.get_sw_name(sw)} yerinde bırakıldı.")
    ...
    def run(self):
        ...
        t3 = threading.Thread(target=self.latency_monitor_loop, daemon=True) # Yeni Thread
        t1.start()
        t2.start()
        t3.start()
        t2.join()
